...
# ── fallback prompts (根据群聊与直播两种模式做语义区隔) ──
...
VOICE_CHATTER_SUB_AGENT_PROMPT_TEMPLATE = SUB_AGENT_PROMPT_VTB
...
async def _decide_via_llm(
    *,
    ...
    bot_id_section = f"它的平台用户标识是 {bot_id}。\n" if bot_id else ""

    from .modes import resolve_mode

    actual_mode = resolve_mode(chat_stream)
    tmpl_name = (
        "anima_chatter_sub_agent_prompt_vtb_live"
        if actual_mode == "vtb_live"
        else "anima_chatter_sub_agent_prompt_vtb"
    )
    tmpl = get_prompt_manager().get_template(tmpl_name)
    if tmpl:
        sub_prompt = (
            await tmpl
        ...
        fallback_tmpl = (
            ...
            if actual_mode == "vtb_live"
            else SUB_AGENT_PROMPT_VTB
        )
        sub_prompt = fallback_tmpl.format(
            nickname=nickname,
            bot_id=bot_id,
