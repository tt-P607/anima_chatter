...
- 关键差异（与普通群聊不一样的地方）：
  ...
  - 群组朋友 / 粉丝可能跟你更熟，可以稍微亲近一点称呼，但**直播间观众也在听**——不能太私密，让陌生观众听了尴尬。
...
## 跨平台找人（关键技巧）
直播间的 ``open_id`` 和群聊里以前记下的平台用户标识（如 ``qq:号码``）是**两套命名空间**——直接用 ``open_id`` 当 ``person_id`` 查，**只能查到这位观众在直播间留下的记忆**，外部群组聊天里那条"张三是个画师"是查不到的。跨平台找人按下面这套用：
